<template>
	<view>
		...
		<uni-list-item title="昵称"class="mt-1">
			<view class="flex align-center" slot="right">
		...
		<uni-list-item title="性别" @click="changeGender"class="mt-1">
			<view class="flex align-center" slot="right">
		...
		<picker mode="date" :value="birthday" @change="onDateChange"class="mt-1" >
		...
		<uni-list-item title="地址" @click="showCityPicker" class="mt-1">
		...
		<view class="py-2 px-3 mt-4">
			<button class="bg-pick text-white rounded-circle" @tap="submit()">
				完成
			</button>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			submit() {
				// 先上传头像
					// this.$http.post('/users/upload', {"file":this.avatar}, 'json').then(res => {
				uni.uploadFile({
					url: 'http://localhost:8081/api/v1/users/upload',
